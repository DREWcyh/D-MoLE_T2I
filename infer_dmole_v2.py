"""Inference entry point for the v2 cross-modal D-MoLE router."""
...
def parse_args():
    """Parse CLI arguments for v2 inference."""
    ...
    parser.add_argument(
        ...
        help="Base prompt used to build residual text features",
    )
...
def main():
    """Run v2 prompt routing and image generation."""
    args = parse_args()
    device = args.device
    ...
    for index, prompt in tqdm(enumerate(prompts), total=len(prompts), desc="Generating images"):
        with torch.no_grad():
            ...
            text_feature = extract_text_features(
                text_hidden_states,
                base_t_input=base_hidden_states,
