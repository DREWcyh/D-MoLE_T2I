"""Inference entry point for the original D-MoLE v1 router."""
...
def parse_args():
    """Parse CLI arguments for v1 inference."""
...
def main():
    """Run v1 prompt routing and image generation."""
    args = parse_args()
    device = args.device
    ...
    with torch.no_grad():
        ...
        base_feature = extract_and_fuse_features(base_hidden_states, latents=None)
        router.set_base_feature(base_feature)

    for index in range(len(task_folders)):
        router.add_task(f"stage{index + 1}")
    ...
    for index, prompt in tqdm(enumerate(prompts), total=len(prompts), desc="Generating images"):
        with torch.no_grad():
            ...
            routing_feature = extract_and_fuse_features(text_hidden_states, latents=None).to(device)
            best_adapter = router.get_top_k_experts(routing_feature, threshold=0.2)

            if best_adapter == "fallback":
                logger.warning(
                    "OOD prompt '%s' did not match a known expert. Falling back to stage1.",
                    prompt,
                )
                best_adapter = "stage1"
        ...
        image.save(os.path.join(args.output_dir, _format_image_filename(index, best_adapter)))
