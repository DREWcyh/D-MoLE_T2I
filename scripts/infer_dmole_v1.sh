...
ADAPTER_ROOT="${ADAPTER_ROOT:-${PROJECT_ROOT}/outputs/train/dmole_v1_autoencoder_router/items_sequential}"
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-${PROJECT_ROOT}/outputs/inference/dmole_v1_autoencoder_router/items_sequential}"
...
for PROMPT_SPEC in "${PROMPT_SPECS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${CUDA_DEVICE}" python "${PROJECT_ROOT}/infer_dmole_v1.py" \
